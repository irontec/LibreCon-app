...
/**
 * Data Mapper implementation for Librecon\Model\KlearRoles
 *
 * @package Librecon\Mapper\Sql
 * @subpackage Raw
 * @author vvargas
...
class KlearRoles extends MapperAbstract
{
    protected $_modelName = 'Librecon\\Model\\KlearRoles';
     ...
     *
     * @param Librecon\Model\Raw\KlearRoles $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Librecon\Model\Raw\KlearRoles) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Librecon\Model\Raw\KlearRoles object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Librecon\Model\\KlearRoles object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Librecon\\Mapper\\Sql\\DbTable\\KlearRoles
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Librecon\\Mapper\\Sql\\DbTable\\KlearRoles');
        }
     ...
     *
     * @param Librecon\Model\Raw\KlearRoles $model The model to delete
    ...
    public function delete(\Librecon\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Librecon\Model\Raw\KlearRoles) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Librecon\\Model\\KlearRoles object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Librecon\\Model\\KlearRoles object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Librecon\\Mapper\\Sql\\DbTable\\KlearRoles', $capitalizedFk);
     ...
     */
    public function save(\Librecon\Model\Raw\KlearRoles $model)
    {
     ...
     * @return boolean If the save action was successful
     */
    public function saveRecursive(\Librecon\Model\Raw\KlearRoles $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Librecon\Model\Raw\KlearRoles $model,
    ...
    )
    {
        ...
        try {
            ...
            if ($recursive) {
                if ($model->getKlearRolesSections(null, null, true) !== null) {
                    $klearRolesSections = $model->getKlearRolesSections();

                    if (!is_array($klearRolesSections)) {

                        $klearRolesSections = array($klearRolesSections);
                    }

                    foreach ($klearRolesSections as $value) {
                        $value->setKlearRoleId($primaryKey)
                              ->saveRecursive(false, $transactionTag);
                    }
                }

                if ($model->getKlearUsersRoles(null, null, true) !== null) {
                    $klearUsersRoles = $model->getKlearUsersRoles();

                    if (!is_array($klearUsersRoles)) {

                        $klearUsersRoles = array($klearUsersRoles);
                    }

                    foreach ($klearUsersRoles as $value) {
                        $value->setKlearRoleId($primaryKey)
                              ->saveRecursive(false, $transactionTag);
                    }
                }
     ...
     * @param Librecon\Model\Raw\KlearRoles|null $entry The object to load the data into, or null to have one created
     * @return Librecon\Model\Raw\KlearRoles The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Librecon\Model\KlearRoles();
        }
        ...
        } else if ($data instanceof \Librecon\Model\Raw\KlearRoles) {
