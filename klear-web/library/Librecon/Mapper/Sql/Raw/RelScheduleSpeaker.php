...
/**
 * Data Mapper implementation for Librecon\Model\RelScheduleSpeaker
 *
 * @package Librecon\Mapper\Sql
 * @subpackage Raw
 * @author vvargas
...
class RelScheduleSpeaker extends MapperAbstract
{
    protected $_modelName = 'Librecon\\Model\\RelScheduleSpeaker';
     ...
     *
     * @param Librecon\Model\Raw\RelScheduleSpeaker $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Librecon\Model\Raw\RelScheduleSpeaker) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Librecon\Model\Raw\RelScheduleSpeaker object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Librecon\Model\\RelScheduleSpeaker object in toArray for " . get_class($this);
            }
        ...
        $result = array(
            'id' => $model->getId(),
            'idSchedule' => $model->getIdSchedule(),
            'idSpeaker' => $model->getIdSpeaker(),
        );
     ...
     *
     * @return Librecon\\Mapper\\Sql\\DbTable\\RelScheduleSpeaker
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Librecon\\Mapper\\Sql\\DbTable\\RelScheduleSpeaker');
        }
     ...
     *
     * @param Librecon\Model\Raw\RelScheduleSpeaker $model The model to delete
    ...
    public function delete(\Librecon\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Librecon\Model\Raw\RelScheduleSpeaker) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Librecon\\Model\\RelScheduleSpeaker object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Librecon\\Model\\RelScheduleSpeaker object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Librecon\\Mapper\\Sql\\DbTable\\RelScheduleSpeaker', $capitalizedFk);
     ...
     */
    public function save(\Librecon\Model\Raw\RelScheduleSpeaker $model)
    {
     ...
     * @return boolean If the save action was successful
     */
    public function saveRecursive(\Librecon\Model\Raw\RelScheduleSpeaker $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Librecon\Model\Raw\RelScheduleSpeaker $model,
     ...
     * @param Librecon\Model\Raw\RelScheduleSpeaker|null $entry The object to load the data into, or null to have one created
     * @return Librecon\Model\Raw\RelScheduleSpeaker The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Librecon\Model\RelScheduleSpeaker();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ->setIdSchedule($data['idSchedule'])
                  ->setIdSpeaker($data['idSpeaker']);
        } else if ($data instanceof \Zend_Db_Table_Row_Abstract || $data instanceof \stdClass) {
            $entry->setId($data->{'id'})
                  ->setIdSchedule($data->{'idSchedule'})
                  ->setIdSpeaker($data->{'idSpeaker'});

        } else if ($data instanceof \Librecon\Model\Raw\RelScheduleSpeaker) {
            $entry->setId($data->getId())
                  ->setIdSchedule($data->getIdSchedule())
                  ->setIdSpeaker($data->getIdSpeaker());
