...
/**
 * Data Mapper implementation for Librecon\Model\KlearRolesSections
 *
 * @package Librecon\Mapper\Sql
 * @subpackage Raw
 * @author vvargas
...
class KlearRolesSections extends MapperAbstract
{
    protected $_modelName = 'Librecon\\Model\\KlearRolesSections';
     ...
     *
     * @param Librecon\Model\Raw\KlearRolesSections $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Librecon\Model\Raw\KlearRolesSections) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Librecon\Model\Raw\KlearRolesSections object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Librecon\Model\\KlearRolesSections object in toArray for " . get_class($this);
            }
        ...
        $result = array(
            'id' => $model->getId(),
            'klearRoleId' => $model->getKlearRoleId(),
            'klearSectionId' => $model->getKlearSectionId(),
        );
     ...
     *
     * @return Librecon\\Mapper\\Sql\\DbTable\\KlearRolesSections
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Librecon\\Mapper\\Sql\\DbTable\\KlearRolesSections');
        }
     ...
     *
     * @param Librecon\Model\Raw\KlearRolesSections $model The model to delete
    ...
    public function delete(\Librecon\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Librecon\Model\Raw\KlearRolesSections) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Librecon\\Model\\KlearRolesSections object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Librecon\\Model\\KlearRolesSections object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Librecon\\Mapper\\Sql\\DbTable\\KlearRolesSections', $capitalizedFk);
     ...
     */
    public function save(\Librecon\Model\Raw\KlearRolesSections $model)
    {
     ...
     * @return boolean If the save action was successful
     */
    public function saveRecursive(\Librecon\Model\Raw\KlearRolesSections $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Librecon\Model\Raw\KlearRolesSections $model,
     ...
     * @param Librecon\Model\Raw\KlearRolesSections|null $entry The object to load the data into, or null to have one created
     * @return Librecon\Model\Raw\KlearRolesSections The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Librecon\Model\KlearRolesSections();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ->setKlearRoleId($data['klearRoleId'])
                  ->setKlearSectionId($data['klearSectionId']);
        } else if ($data instanceof \Zend_Db_Table_Row_Abstract || $data instanceof \stdClass) {
            $entry->setId($data->{'id'})
                  ->setKlearRoleId($data->{'klearRoleId'})
                  ->setKlearSectionId($data->{'klearSectionId'});

        } else if ($data instanceof \Librecon\Model\Raw\KlearRolesSections) {
            $entry->setId($data->getId())
                  ->setKlearRoleId($data->getKlearRoleId())
                  ->setKlearSectionId($data->getKlearSectionId());
