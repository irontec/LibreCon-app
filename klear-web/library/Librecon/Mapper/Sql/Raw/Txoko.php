...
/**
 * Data Mapper implementation for Librecon\Model\Txoko
 *
 * @package Librecon\Mapper\Sql
 * @subpackage Raw
 * @author vvargas
...
class Txoko extends MapperAbstract
{
    protected $_modelName = 'Librecon\\Model\\Txoko';
     ...
     *
     * @param Librecon\Model\Raw\Txoko $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Librecon\Model\Raw\Txoko) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Librecon\Model\Raw\Txoko object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Librecon\Model\\Txoko object in toArray for " . get_class($this);
            }
        ...
        $result = array(
            ...
            'title_en' => $model->getTitleEn(),
            'title_es' => $model->getTitleEs(),
            'orderField' => $model->getOrderField(),
            'text_eu' => $model->getTextEu(),
            'text_en' => $model->getTextEn(),
            'text_es' => $model->getTextEs(),
     ...
     *
     * @return Librecon\\Mapper\\Sql\\DbTable\\Txoko
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Librecon\\Mapper\\Sql\\DbTable\\Txoko');
        }
     ...
     *
     * @param Librecon\Model\Raw\Txoko $model The model to delete
    ...
    public function delete(\Librecon\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Librecon\Model\Raw\Txoko) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Librecon\\Model\\Txoko object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Librecon\\Model\\Txoko object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Librecon\\Mapper\\Sql\\DbTable\\Txoko', $capitalizedFk);
     ...
     */
    public function save(\Librecon\Model\Raw\Txoko $model)
    {
     ...
     * @return boolean If the save action was successful
     */
    public function saveRecursive(\Librecon\Model\Raw\Txoko $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Librecon\Model\Raw\Txoko $model,
     ...
     * @param Librecon\Model\Raw\Txoko|null $entry The object to load the data into, or null to have one created
     * @return Librecon\Model\Raw\Txoko The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Librecon\Model\Txoko();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ...
                  ->setOrderField($data['orderField'])
                  ->setTextEu($data['text_eu'])
        ...
        } else if ($data instanceof \Librecon\Model\Raw\Txoko) {
            $entry->setId($data->getId())
                  ...
                  ->setTitleEn($data->getTitleEn())
                  ->setTitleEs($data->getTitleEs())
                  ->setOrderField($data->getOrderField())
                  ->setTextEu($data->getTextEu())
                  ->setTextEn($data->getTextEn())
                  ->setTextEs($data->getTextEs())
