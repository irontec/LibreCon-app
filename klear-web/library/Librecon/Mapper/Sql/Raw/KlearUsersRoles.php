...
/**
 * Data Mapper implementation for Librecon\Model\KlearUsersRoles
 *
 * @package Librecon\Mapper\Sql
 * @subpackage Raw
 * @author vvargas
...
class KlearUsersRoles extends MapperAbstract
{
    protected $_modelName = 'Librecon\\Model\\KlearUsersRoles';
     ...
     *
     * @param Librecon\Model\Raw\KlearUsersRoles $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Librecon\Model\Raw\KlearUsersRoles) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Librecon\Model\Raw\KlearUsersRoles object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Librecon\Model\\KlearUsersRoles object in toArray for " . get_class($this);
            }
        ...
        $result = array(
            'id' => $model->getId(),
            'klearUserId' => $model->getKlearUserId(),
     ...
     *
     * @return Librecon\\Mapper\\Sql\\DbTable\\KlearUsersRoles
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Librecon\\Mapper\\Sql\\DbTable\\KlearUsersRoles');
        }
     ...
     *
     * @param Librecon\Model\Raw\KlearUsersRoles $model The model to delete
    ...
    public function delete(\Librecon\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Librecon\Model\Raw\KlearUsersRoles) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Librecon\\Model\\KlearUsersRoles object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Librecon\\Model\\KlearUsersRoles object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Librecon\\Mapper\\Sql\\DbTable\\KlearUsersRoles', $capitalizedFk);
     ...
     */
    public function save(\Librecon\Model\Raw\KlearUsersRoles $model)
    {
     ...
     * @return boolean If the save action was successful
     */
    public function saveRecursive(\Librecon\Model\Raw\KlearUsersRoles $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Librecon\Model\Raw\KlearUsersRoles $model,
     ...
     * @param Librecon\Model\Raw\KlearUsersRoles|null $entry The object to load the data into, or null to have one created
     * @return Librecon\Model\Raw\KlearUsersRoles The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Librecon\Model\KlearUsersRoles();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ->setKlearUserId($data['klearUserId'])
        ...
        } else if ($data instanceof \Librecon\Model\Raw\KlearUsersRoles) {
            $entry->setId($data->getId())
                  ->setKlearUserId($data->getKlearUserId())
