...
/**
 * Data Mapper implementation for Librecon\Model\KlearUsers
 *
 * @package Librecon\Mapper\Sql
 * @subpackage Raw
 * @author vvargas
...
{
    protected $_modelName = 'Librecon\\Model\\KlearUsers';
     ...
     *
     * @param Librecon\Model\Raw\KlearUsers $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Librecon\Model\Raw\KlearUsers) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Librecon\Model\Raw\KlearUsers object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Librecon\Model\\KlearUsers object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Librecon\\Mapper\\Sql\\DbTable\\KlearUsers
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Librecon\\Mapper\\Sql\\DbTable\\KlearUsers');
        }
     ...
     *
     * @param Librecon\Model\Raw\KlearUsers $model The model to delete
    ...
    public function delete(\Librecon\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Librecon\Model\Raw\KlearUsers) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Librecon\\Model\\KlearUsers object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Librecon\\Model\\KlearUsers object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Librecon\\Mapper\\Sql\\DbTable\\KlearUsers', $capitalizedFk);
     ...
     */
    public function save(\Librecon\Model\Raw\KlearUsers $model)
    {
     ...
     * @return boolean If the save action was successful
     */
    public function saveRecursive(\Librecon\Model\Raw\KlearUsers $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Librecon\Model\Raw\KlearUsers $model,
            ...
            if ($recursive) {
                if ($model->getKlearUsersRoles(null, null, true) !== null) {
                    ...
                    foreach ($klearUsersRoles as $value) {
                        $value->setKlearUserId($primaryKey)
     ...
     * @param Librecon\Model\Raw\KlearUsers|null $entry The object to load the data into, or null to have one created
     * @return Librecon\Model\Raw\KlearUsers The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Librecon\Model\KlearUsers();
        }
        ...
        } else if ($data instanceof \Librecon\Model\Raw\KlearUsers) {
