...
/**
 * Data Mapper implementation for Librecon\Model\KlearSections
 *
 * @package Librecon\Mapper\Sql
 * @subpackage Raw
 * @author vvargas
...
class KlearSections extends MapperAbstract
{
    protected $_modelName = 'Librecon\\Model\\KlearSections';
     ...
     *
     * @param Librecon\Model\Raw\KlearSections $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Librecon\Model\Raw\KlearSections) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Librecon\Model\Raw\KlearSections object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Librecon\Model\\KlearSections object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Librecon\\Mapper\\Sql\\DbTable\\KlearSections
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Librecon\\Mapper\\Sql\\DbTable\\KlearSections');
        }
     ...
     *
     * @param Librecon\Model\Raw\KlearSections $model The model to delete
    ...
    public function delete(\Librecon\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Librecon\Model\Raw\KlearSections) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Librecon\\Model\\KlearSections object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Librecon\\Model\\KlearSections object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Librecon\\Mapper\\Sql\\DbTable\\KlearSections', $capitalizedFk);
     ...
     */
    public function save(\Librecon\Model\Raw\KlearSections $model)
    {
     ...
     * @return boolean If the save action was successful
     */
    public function saveRecursive(\Librecon\Model\Raw\KlearSections $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Librecon\Model\Raw\KlearSections $model,
            ...
            if ($recursive) {
                if ($model->getKlearRolesSections(null, null, true) !== null) {
                    ...
                    foreach ($klearRolesSections as $value) {
                        $value->setKlearSectionId($primaryKey)
     ...
     * @param Librecon\Model\Raw\KlearSections|null $entry The object to load the data into, or null to have one created
     * @return Librecon\Model\Raw\KlearSections The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Librecon\Model\KlearSections();
        }
        ...
        } else if ($data instanceof \Librecon\Model\Raw\KlearSections) {
