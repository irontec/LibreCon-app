...
class Speaker extends MapperAbstract
{
    protected $_modelName = 'Librecon\\Model\\Speaker';
     ...
     *
     * @param Librecon\Model\Raw\Speaker $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Librecon\Model\Raw\Speaker) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Librecon\Model\Raw\Speaker object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Librecon\Model\\Speaker object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Librecon\\Mapper\\Sql\\DbTable\\Speaker
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Librecon\\Mapper\\Sql\\DbTable\\Speaker');
        }
     ...
     *
     * @param Librecon\Model\Raw\Speaker $model The model to delete
    ...
    public function delete(\Librecon\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Librecon\Model\Raw\Speaker) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Librecon\\Model\\Speaker object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Librecon\\Model\\Speaker object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Librecon\\Mapper\\Sql\\DbTable\\Speaker', $capitalizedFk);
    ...
    protected function _save(\Librecon\Model\Raw\Speaker $model,
            ...
            if ($recursive) {
                if ($model->getRelScheduleSpeaker(null, null, true) !== null) {
                    ...
                    foreach ($relScheduleSpeaker as $value) {
                        $value->setIdSpeaker($primaryKey)
                              ->saveRecursive(false, $transactionTag);
                    }
                }

                if ($model->getRelTagSpeaker(null, null, true) !== null) {
                    $relTagSpeaker = $model->getRelTagSpeaker();

                    if (!is_array($relTagSpeaker)) {

                        $relTagSpeaker = array($relTagSpeaker);
                    }

                    foreach ($relTagSpeaker as $value) {
                        $value->setIdSpeaker($primaryKey)
                              ->saveRecursive(false, $transactionTag);
                    }
                }
     ...
     * @param Librecon\Model\Raw\Speaker|null $entry The object to load the data into, or null to have one created
     * @return Librecon\Model\Raw\Speaker The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Librecon\Model\Speaker();
        }
        ...
        } else if ($data instanceof \Librecon\Model\Raw\Speaker) {
