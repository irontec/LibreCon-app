...
/**
 * Data Mapper implementation for Librecon\Model\Expositor
 *
 * @package Librecon\Mapper\Sql
 * @subpackage Raw
 * @author vvargas
...
class Expositor extends MapperAbstract
{
    protected $_modelName = 'Librecon\\Model\\Expositor';
     ...
     *
     * @param Librecon\Model\Raw\Expositor $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Librecon\Model\Raw\Expositor) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Librecon\Model\Raw\Expositor object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Librecon\Model\\Expositor object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Librecon\\Mapper\\Sql\\DbTable\\Expositor
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Librecon\\Mapper\\Sql\\DbTable\\Expositor');
        }
     ...
     *
     * @param Librecon\Model\Raw\Expositor $model The model to delete
    ...
    public function delete(\Librecon\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Librecon\Model\Raw\Expositor) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Librecon\\Model\\Expositor object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Librecon\\Model\\Expositor object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Librecon\\Mapper\\Sql\\DbTable\\Expositor', $capitalizedFk);
     ...
     */
    public function save(\Librecon\Model\Raw\Expositor $model)
    {
     ...
     * @return boolean If the save action was successful
     */
    public function saveRecursive(\Librecon\Model\Raw\Expositor $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Librecon\Model\Raw\Expositor $model,
     ...
     * @param Librecon\Model\Raw\Expositor|null $entry The object to load the data into, or null to have one created
     * @return Librecon\Model\Raw\Expositor The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Librecon\Model\Expositor();
        }
        ...
        } else if ($data instanceof \Librecon\Model\Raw\Expositor) {
