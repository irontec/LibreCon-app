...
class Meeting extends MapperAbstract
{
    protected $_modelName = 'Librecon\\Model\\Meeting';
     ...
     *
     * @param Librecon\Model\Raw\Meeting $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Librecon\Model\Raw\Meeting) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Librecon\Model\Raw\Meeting object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Librecon\Model\\Meeting object in toArray for " . get_class($this);
            }
        ...
        $result = array(
            ...
            'emailShare' => $model->getEmailShare(),
            'cellphoneShare' => $model->getCellphoneShare(),
            'atRightNow' => $model->getAtRightNow(),
            'atHalfHour' => $model->getAtHalfHour(),
            'atOneHour' => $model->getAtOneHour(),
            'responseDate' => $model->getResponseDate(),
        );
     ...
     *
     * @return Librecon\\Mapper\\Sql\\DbTable\\Meeting
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Librecon\\Mapper\\Sql\\DbTable\\Meeting');
        }
     ...
     *
     * @param Librecon\Model\Raw\Meeting $model The model to delete
    ...
    public function delete(\Librecon\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Librecon\Model\Raw\Meeting) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Librecon\\Model\\Meeting object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Librecon\\Model\\Meeting object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Librecon\\Mapper\\Sql\\DbTable\\Meeting', $capitalizedFk);
     ...
     * @return boolean If the save action was successful
     */
    public function saveRecursive(\Librecon\Model\Raw\Meeting $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Librecon\Model\Raw\Meeting $model,
     ...
     * @param Librecon\Model\Raw\Meeting|null $entry The object to load the data into, or null to have one created
     * @return Librecon\Model\Raw\Meeting The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Librecon\Model\Meeting();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ...
                  ->setEmailShare($data['emailShare'])
                  ->setCellphoneShare($data['cellphoneShare'])
                  ->setAtRightNow($data['atRightNow'])
                  ->setAtHalfHour($data['atHalfHour'])
                  ->setAtOneHour($data['atOneHour'])
        ...
        } else if ($data instanceof \Librecon\Model\Raw\Meeting) {
            $entry->setId($data->getId())
                  ->setSender($data->getSender())
                  ->setReceiver($data->getReceiver())
                  ->setCreatedAt($data->getCreatedAt())
                  ->setStatus($data->getStatus())
                  ->setEmailShare($data->getEmailShare())
                  ->setCellphoneShare($data->getCellphoneShare())
                  ->setAtRightNow($data->getAtRightNow())
                  ->setAtHalfHour($data->getAtHalfHour())
                  ->setAtOneHour($data->getAtOneHour())
                  ->setResponseDate($data->getResponseDate());
