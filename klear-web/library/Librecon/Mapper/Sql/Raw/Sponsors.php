...
/**
 * Data Mapper implementation for Librecon\Model\Sponsors
 *
 * @package Librecon\Mapper\Sql
 * @subpackage Raw
 * @author vvargas
...
class Sponsors extends MapperAbstract
{
    protected $_modelName = 'Librecon\\Model\\Sponsors';
     ...
     *
     * @param Librecon\Model\Raw\Sponsors $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Librecon\Model\Raw\Sponsors) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Librecon\Model\Raw\Sponsors object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Librecon\Model\\Sponsors object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Librecon\\Mapper\\Sql\\DbTable\\Sponsors
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Librecon\\Mapper\\Sql\\DbTable\\Sponsors');
        }
     ...
     *
     * @param Librecon\Model\Raw\Sponsors $model The model to delete
    ...
    public function delete(\Librecon\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Librecon\Model\Raw\Sponsors) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Librecon\\Model\\Sponsors object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Librecon\\Model\\Sponsors object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Librecon\\Mapper\\Sql\\DbTable\\Sponsors', $capitalizedFk);
     ...
     */
    public function save(\Librecon\Model\Raw\Sponsors $model)
    {
     ...
     * @return boolean If the save action was successful
     */
    public function saveRecursive(\Librecon\Model\Raw\Sponsors $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Librecon\Model\Raw\Sponsors $model,
     ...
     * @param Librecon\Model\Raw\Sponsors|null $entry The object to load the data into, or null to have one created
     * @return Librecon\Model\Raw\Sponsors The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Librecon\Model\Sponsors();
        }
        ...
        } else if ($data instanceof \Librecon\Model\Raw\Sponsors) {
