...
/**
 * Data Mapper implementation for Librecon\Model\RelTagScheduele
 *
 * @package Librecon\Mapper\Sql
 * @subpackage Raw
 * @author vvargas
...
class RelTagScheduele extends MapperAbstract
{
    protected $_modelName = 'Librecon\\Model\\RelTagScheduele';
     ...
     *
     * @param Librecon\Model\Raw\RelTagScheduele $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Librecon\Model\Raw\RelTagScheduele) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Librecon\Model\Raw\RelTagScheduele object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Librecon\Model\\RelTagScheduele object in toArray for " . get_class($this);
            }
        ...
        $result = array(
            ...
            'idScheduele' => $model->getIdScheduele(),
        );
     ...
     *
     * @return Librecon\\Mapper\\Sql\\DbTable\\RelTagScheduele
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Librecon\\Mapper\\Sql\\DbTable\\RelTagScheduele');
        }
     ...
     *
     * @param Librecon\Model\Raw\RelTagScheduele $model The model to delete
    ...
    public function delete(\Librecon\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Librecon\Model\Raw\RelTagScheduele) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Librecon\\Model\\RelTagScheduele object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Librecon\\Model\\RelTagScheduele object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Librecon\\Mapper\\Sql\\DbTable\\RelTagScheduele', $capitalizedFk);
     ...
     */
    public function save(\Librecon\Model\Raw\RelTagScheduele $model)
    {
     ...
     * @return boolean If the save action was successful
     */
    public function saveRecursive(\Librecon\Model\Raw\RelTagScheduele $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Librecon\Model\Raw\RelTagScheduele $model,
     ...
     * @param Librecon\Model\Raw\RelTagScheduele|null $entry The object to load the data into, or null to have one created
     * @return Librecon\Model\Raw\RelTagScheduele The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Librecon\Model\RelTagScheduele();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ->setIdTag($data['idTag'])
                  ->setIdScheduele($data['idScheduele']);
        } else if ($data instanceof \Zend_Db_Table_Row_Abstract || $data instanceof \stdClass) {
            $entry->setId($data->{'id'})
                  ->setIdTag($data->{'idTag'})
                  ->setIdScheduele($data->{'idScheduele'});

        } else if ($data instanceof \Librecon\Model\Raw\RelTagScheduele) {
            $entry->setId($data->getId())
                  ->setIdTag($data->getIdTag())
                  ->setIdScheduele($data->getIdScheduele());
