...
{
    protected $_modelName = 'Librecon\\Model\\Tags';
     ...
     *
     * @param Librecon\Model\Raw\Tags $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Librecon\Model\Raw\Tags) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Librecon\Model\Raw\Tags object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Librecon\Model\\Tags object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Librecon\\Mapper\\Sql\\DbTable\\Tags
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Librecon\\Mapper\\Sql\\DbTable\\Tags');
        }
     ...
     *
     * @param Librecon\Model\Raw\Tags $model The model to delete
    ...
    public function delete(\Librecon\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Librecon\Model\Raw\Tags) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Librecon\\Model\\Tags object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Librecon\\Model\\Tags object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Librecon\\Mapper\\Sql\\DbTable\\Tags', $capitalizedFk);
    ...
    protected function _save(\Librecon\Model\Raw\Tags $model,
     ...
     * @param Librecon\Model\Raw\Tags|null $entry The object to load the data into, or null to have one created
     * @return Librecon\Model\Raw\Tags The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Librecon\Model\Tags();
        }
        ...
        } else if ($data instanceof \Librecon\Model\Raw\Tags) {
